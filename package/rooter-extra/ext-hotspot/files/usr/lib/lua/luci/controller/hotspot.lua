...
function action_check_spot()
	...
	rv['bstate'] = luci.model.uci.cursor():get("basic", "basic", "state")
	if rv['bstate'] == "" then
		rv['bstate'] = '0'
	end
	auto = luci.model.uci.cursor():get("travelmate", "global", "trm_auto")
	rv["auto"] = auto
	rv["enabled"] = luci.model.uci.cursor():get("travelmate", "global", "state")
	if rv["enabled"] == "" then
		rv["enabled"] = "0"
	end
	...
	currad = luci.model.uci.cursor():get("travelmate", "global", "currrad")
	encr = luci.model.uci.cursor():get("wireless", "wwan" .. currad, "encryption")
	if encr == "none" then
		rv["encryp"] = translate("Open")
	else
		rv["encryp"] = translate("Encrypted")
	end
	rv["disable"] = luci.model.uci.cursor():get("wireless", "wwan" .. currad, "disabled")
	...
	device = luci.model.uci.cursor():get("wireless", "wwan" .. currad, "device")
